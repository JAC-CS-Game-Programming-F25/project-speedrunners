 ...
 * @param {number} margin - Margin around the entire tileset (default 0)
 * @param {number} spacing - Spacing between tiles (default 0)
 * @returns
 */
static generateSpritesFromSpriteSheet(spriteSheet, tileWidth, tileHeight, margin = 0, spacing = 0) {
    const sprites = [];
    
    // Calculate usable dimensions after accounting for margin
    const usableWidth = spriteSheet.width - margin;
    const usableHeight = spriteSheet.height - margin;
    
    const sheetWidth = Math.floor(usableWidth / (tileWidth + spacing));
    const sheetHeight = Math.floor(usableHeight / (tileHeight + spacing));
